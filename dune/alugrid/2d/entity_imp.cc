...
template<int cd, int dim, class GridImp>
inline ALU2dGridEntity<cd, dim, GridImp> ::
ALU2dGridEntity(const FactoryType& factory, const ElementType &element, int face, int level)
  : factory_( &factory )
{
  setElement( element, face, level );
}
...
template<int dim, class GridImp>
inline ALU2dGridEntity<0,dim,GridImp> ::
ALU2dGridEntity(const FactoryType& factory, const HElementType &item, int face, int level)
  : factory_( &factory )
{
  setElement( item, face, level );
}
...
inline typename ALU2dGridEntity<0, dim, GridImp> :: EntityPointer ALU2dGridEntity<0,dim,GridImp> :: father () const
{
  ...
  return EntityPointerImp(factory(), *(item_->father()));
}
...
struct ElementWrapper<0,dim, GridImp>
{
  ...
  typedef typename GridImp::Traits::template Codim<0>::EntityPointerImp  EntityPointerImp;
  ...
  static inline EntityPointer
  subEntity(const FactoryType& factory, const HElementType &elem, int i)
  {
    //alugrid_assert (!i);
    return EntityPointerImp(factory, elem, -1, elem.level());
  }
...
struct ElementWrapper<1, dim, GridImp>{
  ...
  typedef typename ALU2dImplTraits< GridImp::dimensionworld, GridImp::elementType >::HElementType HElementType ;
  typedef typename GridImp :: GridObjectFactoryType  FactoryType;
  typedef typename GridImp::Traits::template Codim<1>::EntityPointerImp  EntityPointerImp;
  typedef typename GridImp::Traits::template Codim<1>::EntityPointer     EntityPointer;
  ...
  {
    alugrid_assert (i < elem.numvertices() && i >= 0);
    return EntityPointerImp(factory, elem, i, elem.level());
  }
  static inline int subBoundary(GridImp & grid, const HElementType &elem, int i) {
...
struct ElementWrapper<2, dim, GridImp>{
  ...
  typedef typename GridImp :: GridObjectFactoryType  FactoryType;
  typedef typename GridImp::Traits::template Codim<2>::EntityPointerImp  EntityPointerImp;
  typedef typename GridImp::Traits::template Codim<2>::EntityPointer     EntityPointer;
  ...
  {
    alugrid_assert (i < elem.numvertices() && i >= 0);
    return EntityPointerImp(factory, *(elem.getVertex(i)), -1, elem.level());
  }
  static inline int subBoundary(GridImp & grid, const HElementType &elem, int i) {
