  ...
  struct ALU2dGridFamily
  {
    ...
    struct Traits
    {
      ...
      struct Codim
      {
        ...
        typedef ALU2dGridEntity< cd, dim, const GridImp > EntityImp;
        typedef Dune::Entity<cd, dim, const GridImp, ALU2dGridEntity> Entity;

#if DUNE_VERSION_NEWER(DUNE_GRID,2,4)
        typedef EntityImp  EntityPointerImp;
        typedef Entity     EntityPointer;
#else
        typedef ALU2dGridEntityPointer< cd, const GridImp > EntityPointerImp;
        typedef Dune::EntityPointer< const GridImp, EntityPointerImp > EntityPointer;
#endif
  ...
  */
  ...
  public:
    ...
    entityPointer( const EntitySeed& seed ) const
    {
      ...
      typedef typename Traits::template Codim<codim>::EntityPointerImp EntityPointerImp;
      return EntityPointerImp( factory(), *(seed.item()), seed.face(), seed.level() ) ;
    }
  ...
  public:
    ...
    */
    ...
    /** \brief @copydoc Dune::Grid::loadBalance */
    template<class DataHandle>
    bool repartition(DataHandle& data) { return loadBalance( data ); }
